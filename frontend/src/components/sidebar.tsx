    ...
    <div className="relative shrink-0 w-full mb-2">
      <div className="flex items-center p-1 w-full">
        ...
        <div className="px-2 py-1 flex flex-col justify-center">
          <div className="font-lexend font-semibold text-[16px] text-neutral-50 leading-tight">
            Exodus AI
          </div>
...
function AvatarCircle() {
  return (
    <div className="relative rounded-full shrink-0 size-8 bg-neutral-900 border border-neutral-800 flex items-center justify-center">
...
function SearchContainer({ isCollapsed = false }: { isCollapsed?: boolean }) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "bg-neutral-900 h-10 relative rounded-lg flex items-center transition-all duration-500 border border-neutral-800/50",
      ...
      >
        ...
        {!isCollapsed && (
          <input
            ...
            className="flex-1 bg-transparent border-none outline-none font-lexend text-[13px] text-neutral-50 placeholder:text-neutral-600 pr-3"
          />
        )}
      </div>
    </div>
...
const historyItems: HistoryItem[] = [
  { job_id: '1', source_label: 'Quarterly Sales Data', overall_risk: 'Low' },
  { job_id: '2', source_label: 'Inventory_v2.csv', overall_risk: 'Medium' },
  { job_id: '3', source_label: 'Churn_Analysis_2024', overall_risk: 'High' },
];
...
function RiskDot({ risk }: { risk?: string }) {
  ...
  return <div className={cn("size-1.5 rounded-full", colors[risk as keyof typeof colors] || 'bg-neutral-600')} />;
}
...
function IconNavButton({
  ...
  return (
    <button
      type="button"
      className={cn(
        "flex items-center justify-center rounded-xl size-10 min-w-10 transition-all duration-500 cursor-pointer",
        isActive ? "bg-neutral-800 text-neutral-50" : "hover:bg-neutral-900 text-neutral-500 hover:text-neutral-300"
      )}
      style={{ transitionTimingFunction: softSpringEasing }}
      onClick={onClick}
    >
...
function MenuItem({
  ...
  const content = (
    <div
      className={cn(
        "group flex items-center gap-3 px-2 py-2 w-full rounded-xl transition-all duration-300 relative",
        isActive ? "bg-neutral-800/50 text-neutral-50" : "hover:bg-neutral-900/50 text-neutral-400 hover:text-neutral-200"
      )}
    >
      <div className="shrink-0 flex items-center justify-center size-5">
        {item.risk ? <RiskDot risk={item.risk} /> : (item.icon || <Folder size={16} />)}
      </div>

      {!isCollapsed && (
        <>
          <span className="flex-1 font-lexend text-[13px] truncate">
            {item.label}
          </span>
          {item.hasDropdown && (
            <ChevronDownIcon
      ...
      {isActive && !isCollapsed && (
        <div className="absolute left-0 w-1 h-4 bg-neutral-50 rounded-full" />
      )}
    </div>
  );

  if (item.href && !item.hasDropdown) {
    return <Link href={item.href} className="w-full">{content}</Link>;
  }
...
export function Sidebar({ 
  ...
  const sections = [
    { id: 'analyses', icon: <Catalog size={18} />, label: 'Analyses' },
    { id: 'data', icon: <DataCategorical size={18} />, label: 'Data Hub' },
    { id: 'analytics', icon: <ChartBar size={18} />, label: 'Insights' },
    { id: 'settings', icon: <SettingsIcon size={18} />, label: 'Settings' },
  ];

  return (
    <div className="fixed left-0 top-0 h-screen flex z-50 p-2 gap-2 bg-black">
      {/* Left Navigation Rail */}
      <aside className="bg-neutral-950 flex flex-col gap-3 items-center p-3 w-16 h-full border border-neutral-800/50 rounded-2xl shadow-2xl">
        <div className="mb-4 size-10 flex items-center justify-center p-1 bg-neutral-900 rounded-xl border border-neutral-800">
          <ExodusLogoSquare />
        </div>
      ...
      <aside
        className={cn(
          "bg-neutral-950 flex flex-col gap-6 p-4 rounded-2xl border border-neutral-800/50 shadow-2xl transition-all duration-500",
          isCollapsed ? "w-16 !p-2" : "w-72"
        )}
        style={{ transitionTimingFunction: softSpringEasing }}
      >
        {!isCollapsed && (
          <div className="flex items-center justify-between">
            ...
            <button 
              ...
              className="p-2 hover:bg-neutral-900 rounded-lg text-neutral-500 cursor-pointer"
            >
        ...
        {isCollapsed && (
          <button 
            ...
            className="w-full flex justify-center p-2 hover:bg-neutral-900 rounded-lg text-neutral-500 mb-4 cursor-pointer"
          >
            <ChevronDownIcon size={16} className="-rotate-90" />
          </button>
        )}
        ...
        {/* Action Button */}
        {!isCollapsed && (
          <button className="flex items-center justify-center gap-2 w-full py-2.5 bg-neutral-50 text-neutral-950 rounded-xl font-lexend font-semibold text-[13px] hover:bg-neutral-200 transition-colors shadow-lg shadow-white/5 cursor-pointer">
            <AddLarge size={16} />
            New Analysis
          </button>
        )}
        ...
        <div className="flex-1 flex flex-col gap-8 overflow-y-auto custom-scrollbar">
          ...
          <div className="flex flex-col gap-2">
            {!isCollapsed && (
              <div className="flex items-center justify-between px-2 mb-1">
                <span className="font-lexend font-medium text-[11px] text-neutral-500 uppercase tracking-widest">
                  Recent Analyses
                </span>
                <Filter size={12} className="text-neutral-600 hover:text-neutral-400 cursor-pointer" />
              </div>
            )}
            ...
            <div className="flex flex-col gap-0.5">
              {historyItems.map((item) => (
                <MenuItem
                  key={item.job_id}
                  item={{
                    label: item.source_label || item.source || 'Untitled',
          ...
          {/* Quick Sections (only if not collapsed) */}
          {!isCollapsed && (
            <div className="flex flex-col gap-2">
              <span className="font-lexend font-medium text-[11px] text-neutral-500 uppercase tracking-widest px-2 mb-1">
                Starred
              </span>
              <MenuItem
                item={{ label: 'Monthly Report.xlsx', risk: 'Low' }}
                isCollapsed={false}
                pathname={pathname}
              />
            </div>
        ...
        {/* Footer */}
        <div className={cn(
          "mt-auto pt-4 border-t border-neutral-900 flex flex-col gap-1",
          isCollapsed ? "items-center" : ""
        )}>
          {!isCollapsed ? (
            <>
              <button className="flex items-center gap-3 px-3 py-2 w-full text-neutral-500 hover:text-neutral-300 hover:bg-neutral-900/50 rounded-xl transition-all cursor-pointer">
                <View size={16} />
                <span className="font-lexend text-[13px]">Theme Mode</span>
              </button>
              <button className="flex items-center gap-3 px-3 py-2 w-full text-red-500/70 hover:text-red-500 hover:bg-red-500/5 rounded-xl transition-all cursor-pointer">
                <Logout size={16} />
                <span className="font-lexend text-[13px]">Sign Out</span>
              </button>
            </>
          ) : (
            <button className="p-2 hover:bg-neutral-900 rounded-lg text-neutral-500">
              <Logout size={16} />
            </button>
          )}
        </div>
